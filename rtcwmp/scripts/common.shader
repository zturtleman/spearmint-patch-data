textures/common/nodrawnonsolid
{
    surfaceparm nodraw
    surfaceparm nonsolid
}

textures/common/teleporter
{
    q3map_lightimage textures/sfx/powerupshit.tga
    q3map_surfacelight 800
    surfaceparm noimpact
    surfaceparm nolightmap
    {
        //tcMod scale 5 5
        map textures/sfx/powerupshit.tga
        tcGen environment
        tcMod turb 0 0.015 0 0.3
    }
}

textures/common/nodrop
{
    qer_nocarve
    qer_trans 0.5
    surfaceparm nodraw
    surfaceparm nomarks
    surfaceparm nolightmap
    surfaceparm nonsolid
    surfaceparm trans
    surfaceparm nodrop
    cull disable
}

textures/common/clipweap
{
    //surfaceparm playerclip
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm nomarks
    surfaceparm trans
}

textures/common/clipfull
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm playerclip
}

textures/common/clip
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm noimpact
    surfaceparm nomarks
    surfaceparm nonsolid
    surfaceparm playerclip
}

textures/common/clip_metal
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm nomarks
    surfaceparm playerclip
    surfaceparm clipmissile
    surfaceparm metalsteps
}

textures/common/clipmonster
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm noimpact
    surfaceparm nomarks
    surfaceparm nonsolid
    surfaceparm monsterclip
}

textures/common/ladder
{
    //surfaceparm nodamage
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm noimpact
    surfaceparm nomarks
    surfaceparm nonsolid
    surfaceparm playerclip
    surfaceparm ladder
}

textures/common/clipmissile
{ 
	qer_trans 0.3
	surfaceparm nodraw
	surfaceparm nomarks
	surfaceparm playerclip
	surfaceparm trans
	surfaceparm clipmissile
}

textures/common/cushion
{
    qer_nocarve
    qer_trans 0.5
    surfaceparm nodamage
    surfaceparm nodraw
    surfaceparm nomarks
    surfaceparm trans
}

textures/common/hint
{
    qer_nocarve
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm noimpact
    surfaceparm nonsolid
    surfaceparm structural
    surfaceparm trans
}

textures/common/skip
{
    qer_nocarve
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm nonsolid
    surfaceparm structural
    surfaceparm trans
}

textures/common/slick
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm nomarks
    surfaceparm slick
    surfaceparm trans
}

// metal death slip
textures/common/slip_m_n
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm metalsteps
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm trans
}

textures/common/slip_m_ne
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm metalsteps
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_m_e
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm metalsteps
    surfaceparm monsterslick
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_m_se
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm metalsteps
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_m_s
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm metalsteps
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm trans
}

textures/common/slip_m_sw
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm metalsteps
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm monsterslickwest
    surfaceparm trans
}

textures/common/slip_m_w
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm metalsteps
    surfaceparm monsterslick
    surfaceparm monsterslickwest
    surfaceparm trans
}

textures/common/slip_m_nw
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm metalsteps
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm monsterslickwest
    surfaceparm trans
}

// stone death slip
textures/common/slip_n
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm trans
}

textures/common/slip_ne
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_e
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm monsterslick
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_se
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_s
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm trans
}

textures/common/slip_sw
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm monsterslickwest
    surfaceparm trans
}

textures/common/slip_w
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm monsterslick
    surfaceparm monsterslickwest
    surfaceparm trans
}

textures/common/slip_nw
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm monsterslickwest
    surfaceparm trans
}

// wood death slip
textures/common/slip_w_n
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm woodsteps
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm trans
}

textures/common/slip_w_ne
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm woodsteps
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_w_e
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm woodsteps
    surfaceparm monsterslick
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_w_se
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm woodsteps
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_w_s
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm woodsteps
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm trans
}

textures/common/slip_w_sw
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm woodsteps
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm monsterslickwest
    surfaceparm trans
}

textures/common/slip_w_w
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm woodsteps
    surfaceparm monsterslick
    surfaceparm monsterslickwest
    surfaceparm trans
}

textures/common/slip_w_nw
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm woodsteps
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm monsterslickwest
    surfaceparm trans
}

// carpet death slip
textures/common/slip_c_n
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm carpetsteps
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm trans
}

textures/common/slip_c_ne
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm carpetsteps
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_c_e
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm carpetsteps
    surfaceparm monsterslick
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_c_se
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm carpetsteps
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_c_s
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm carpetsteps
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm trans
}

textures/common/slip_c_sw
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm carpetsteps
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm monsterslickwest
    surfaceparm trans
}

textures/common/slip_c_w
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm carpetsteps
    surfaceparm monsterslick
    surfaceparm monsterslickwest
    surfaceparm trans
}

textures/common/slip_c_nw
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm carpetsteps
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm monsterslickwest
    surfaceparm trans
}

// grass death slip
textures/common/slip_g_n
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm grasssteps
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm trans
}

textures/common/slip_g_ne
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm grasssteps
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_g_e
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm grasssteps
    surfaceparm monsterslick
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_g_se
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm grasssteps
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm monsterslickeast
    surfaceparm trans
}

textures/common/slip_g_s
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm grasssteps
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm trans
}

textures/common/slip_g_sw
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm grasssteps
    surfaceparm monsterslick
    surfaceparm monsterslicksouth
    surfaceparm monsterslickwest
    surfaceparm trans
}

textures/common/slip_g_w
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm grasssteps
    surfaceparm monsterslick
    surfaceparm monsterslickwest
    surfaceparm trans
}

textures/common/slip_g_nw
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm grasssteps
    surfaceparm monsterslick
    surfaceparm monsterslicknorth
    surfaceparm monsterslickwest
    surfaceparm trans
}

textures/common/origin
{
    qer_nocarve
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm nonsolid
    surfaceparm origin
}

//bot specific cluster portal
textures/common/clusterportal
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm nomarks
    surfaceparm nonsolid
    surfaceparm trans
    surfaceparm detail
    surfaceparm clusterportal
}

textures/common/areaportal
{
    surfaceparm areaportal
    surfaceparm nodraw
    surfaceparm nomarks
    surfaceparm nonsolid
    surfaceparm structural
    surfaceparm trans
}

textures/common/trigger
{
    qer_nocarve
    qer_trans 0.5
    surfaceparm nodraw
}

textures/common/caulk
{
    surfaceparm nodraw
    surfaceparm nomarks
}

textures/common/lightgrid
{
    qer_trans 0.5
	surfaceparm lightgrid
	surfaceparm nodraw
	surfaceparm nomarks
	surfaceparm nonsolid
	surfaceparm trans
}

textures/common/nodraw
{
    qer_trans 0.3
    surfaceparm nodraw
    surfaceparm nomarks
    surfaceparm nonsolid
    surfaceparm trans
}

textures/common/mirror
{
    surfaceparm nolightmap
    portal
    {
        map textures/common/mirror1.tga
        blendFunc GL_ONE GL_ONE
        depthWrite
    }
}

textures/common/terrain
{
	surfaceparm grasssteps
	surfaceparm nodraw
	surfaceparm nomarks
    	surfaceparm nolightmap
}

textures/common/terrain2
{
	surfaceparm grasssteps
	surfaceparm dust
	surfaceparm nodraw
	surfaceparm nomarks
	surfaceparm nolightmap
}

textures/common/snow_terrain
{
	surfaceparm snowsteps
	surfaceparm nodraw
	surfaceparm nomarks
	surfaceparm nolightmap
}

textures/common/dirtymirror
{
	//dirt pass
	qer_editorimage textures/common/dirtymirror.tga
	surfaceparm nolightmap
	portal
	{
		map textures/common/mirror1.tga
		blendFunc GL_ONE GL_ONE
 		depthWrite
	}
	{
		map textures/common/dirtymirror.tga
		blendFunc GL_ONE_MINUS_SRC_ALPHA GL_SRC_ALPHA
	}
}

textures/common/dirtymirror1a
{
    //dirt pass
    deformVertexes wave 300 sin 0 1 0 0.5
    qer_editorimage textures/common/dirtymirror.tga
    surfaceparm slag    
    portal

    {
		map textures/common/mirror1a.tga
		blendFunc GL_ONE_MINUS_SRC_ALPHA GL_SRC_ALPHA
		depthWrite
    }
    {
		map textures/common/dirtymirrora.tga
		blendFunc GL_ONE_MINUS_SRC_ALPHA GL_SRC_ALPHA
		tcMod stretch sin 1 0.02 0 .05
    }
}

textures/liquids/slag
{
	//dirt pass
	deformVertexes wave 100 sin 0 1 0 0.5
	qer_editorimage textures/common/dirtymirrora.tga
	surfaceparm slag    
	portal
	{
		map textures/common/mirror1a.tga
		blendFunc GL_ONE_MINUS_SRC_ALPHA GL_SRC_ALPHA
		depthWrite
	}

	{
		map textures/common/dirtymirrora.tga
		blendFunc GL_ONE_MINUS_SRC_ALPHA GL_SRC_ALPHA
		tcMod stretch sin 1 0.02 0 .05
	}
}

textures/common/energypad
{
    qer_editorimage textures/common/bluegoal.tga
    surfaceparm nolightmap
    cull disable
    {
        map textures/common/bluegoal.tga
        blendFunc GL_ONE GL_SRC_ALPHA
        tcGen environment
        tcMod turb 0 0.25 0 0.05
    }
}

textures/common/portal
{
	qer_editorimage textures/common/mirror1.tga
	surfaceparm nolightmap
	portal
	{
		map textures/common/mirror1.tga
 		blendFunc GL_ONE GL_ONE_MINUS_SRC_ALPHA
		depthWrite
		alphaGen portal 2048
		tcMod turb 0 0.25 0 0.05
    }
}

textures/common/portaltrans
{
	qer_editorimage textures/common/mirror1.tga
	surfaceparm nolightmap
	surfaceparm trans // So that light will pass through the portal instead of blocking light.
	portal
	{
		map textures/common/mirror1.tga
 		blendFunc GL_ONE GL_ONE_MINUS_SRC_ALPHA
		depthWrite
		alphaGen portal 2048
		tcMod turb 0 0.25 0 0.05
	}
}

textures/common/portal1
{
	qer_editorimage textures/common/portal1.tga
	surfaceparm nolightmap
	portal
	{
		map textures/common/portal_grad1.tga
		blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
		depthWrite
	}
}

textures/sfx/portal_sfx
{
	//******************************************
	//*     Portal Inside Effect               *
	//*     April 29                           *
	//*     Please Comment Changes             *
	//******************************************
	surfaceparm nolightmap
	deformVertexes wave 100 sin 0 2 0 0.5
	portal
	{
	//This blend function is not working on RagePro drivers Mac only
	//when it is used on top of portals
		map textures/sfx/portal_sfx3.tga
		blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
		depthWrite
	}
	{
		map textures/sfx/portal_sfx1.tga
		blendFunc GL_DST_COLOR GL_ZERO
		tcMod rotate 360
	}
	{
		map textures/sfx/portal_sfx.tga
		blendFunc GL_ONE GL_ONE
		rgbGen wave inversesawtooth 0 0.5 0.2 0.5
	}
	{
		map textures/sfx/portalfog.tga
		blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
		rgbGen identityLighting
		alphaGen portal 256
		tcMod turb 0 1 0 1
		tcMod rotate 0.1
		tcMod scroll 0.01 0.03
	}
}

textures/sfx/portal2
{
	deformVertexes wave 100 sin 0 10 0 0.5
	surfaceparm nolightmap
	portal
	{
	//This blend function is not working on RagePro drivers Mac only
	//when it is used on top of portals
		map textures/sfx/portal2.tga
		blendFunc GL_ONE GL_ONE_MINUS_SRC_ALPHA
		depthWrite
	}
}

textures/sfx/portal3a_back2

{
    surfaceparm nolightmap
    deformVertexes wave 100 sin 0 2 0 0.5
    portal
    {
        //This blend function is not working on RagePro drivers Mac only
        //when it is used on top of portals
        map textures/sfx/portal_sfx3.tga
        blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
        depthWrite
    }
    {
        map textures/sfx/portal_sfx1.tga
        blendFunc GL_DST_COLOR GL_ZERO
   //     tcMod rotate 360azz    }
	} // ZTM: lets add missing brace!
    {
        map textures/sfx/portal_sfx.tga
        blendFunc GL_ONE GL_ONE
        rgbGen wave inversesawtooth 0 0.5 0.2 0.5
    }
    {
        //* sin 0 .5 0 1
        map textures/sfx/portalfog.tga
        blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
        rgbGen identityLighting
        alphaGen portal 24
       // tcMod turb 0 1 0 1
       // tcMod rotate 0.1
       // tcMod scroll 0.01 0.03
    }
}

textures/sfx/portal3a
{
    surfaceparm nolightmap
    deformVertexes wave 100 sin 0 10 0 0.5
    surfaceparm trans
    portal
    {
        //This blend function is not working on RagePro drivers Mac only
        //when it is used on top of portals
        map textures/sfx/portal3a.tga
        blendFunc GL_ONE GL_ONE_MINUS_SRC_ALPHA 
        depthWrite
        blendFunc GL_ONE GL_ONE
        rgbGen wave inversesawtooth .5 0 0.2 0.5
    }
    {
        //* sin 0 .5 0 1
        map textures/sfx/portal3a.tga
        blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
        rgbGen identityLighting
       alphaGen portal 1024
       // tcMod turb 0 1 0 1
       // tcMod rotate 0.1
       // tcMod scroll 0.01 0.03
    }
}

textures/sfx/portal3
{
    deformVertexes wave 100 sin 0 10 0 0.5
    //portal
    surfaceparm nolightmap
    surfaceparm trans
    {
        //This blend function is not working on RagePro drivers Mac only
        //when it is used on top of portals
        map textures/sfx/portal3.tga
        blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
        depthFunc disable
        depthWrite
	rgbGen identity
    }
}

textures/sfx/grad
{
    deformVertexes wave 100 sin 0 10 0 0.5
    //portal
    surfaceparm nolightmap
    {
        //This blend function is not working on RagePro drivers Mac only
        //when it is used on top of portals
        map textures/sfx/grad.tga
        blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
        depthFunc disable
        depthWrite
    }
}

textures/sfx/helga_portal
{
	surfaceparm trans
	surfaceparm noimpact
	surfaceparm nolightmap
	tesssize 128
	deformVertexes wave 160 sin 0 10 0 .3	// the chop
	{
		map textures/sfx/helga_portal.tga
		blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
//		depthWrite
		tcMod rotate 6	
	}
}

textures/sfx/helga_skyboxportal
{
	qer_editorimage textures/sfx/helga_portal.tga
	surfaceparm trans
	surfaceparm nolightmap
	qer_nocarve
	{
		map textures/sfx/helga_portal.tga
		blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
//		depthWrite
		tcMod rotate 8
	}
}

textures/sfx/helga_portal			// SMF
{
	qer_editorimage textures/sfx/helga_portal.tga
	surfaceparm trans
	surfaceparm nolightmap
	surfaceparm nonsolid
	tesssize 128

	deformVertexes wave 160 sin 0 10 0 .3	// the chop

    {
        map textures/sfx/helga_portal.tga
		blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
        tcMod rotate 12
    }
}

textures/sfx/helga_portallocal
{
//	portal
	qer_editorimage textures/sfx/helga_portal.tga
	surfaceparm trans
	surfaceparm nolightmap
		surfaceparm nonsolid
	tesssize 128
		
	deformVertexes wave 160 sin 0 10 0 .3	// the chop
//	deformVertexes wave 25 sin 0 5 0 .1	// the chop
//	deformVertexes wave 64 sin .25 .25 0 .5

    {
        map textures/sfx/helga_portal.tga
	blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
//	depthWrite
        tcMod rotate 12
//	tcMod rotate 8
	
    }
}

textures/sfx/helga_portallocal2
{
//	portal
	qer_editorimage textures/sfx/helga_portal.tga
	surfaceparm trans
	surfaceparm nolightmap
		surfaceparm nonsolid
	tesssize 128
		
	deformVertexes wave 160 sin 0 10 0 .3	// the chop
//	deformVertexes wave 25 sin 0 5 0 .1	// the chop
//	deformVertexes wave 64 sin .25 .25 0 .5

    {
        map textures/sfx/helga_portal.tga
	blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
//	depthWrite
        tcMod rotate 12
//	tcMod rotate 8
	
    }
}

textures/sfx/helga_portallocal3
{
//	portal
	qer_editorimage textures/sfx/helga_portal.tga
	surfaceparm trans
	surfaceparm nolightmap
		surfaceparm nonsolid
	tesssize 128
		
	deformVertexes wave 160 sin 0 10 0 .3	// the chop
//	deformVertexes wave 25 sin 0 5 0 .1	// the chop
//	deformVertexes wave 64 sin .25 .25 0 .5

    {
        map textures/sfx/helga_portal.tga
	blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
//	depthWrite
        tcMod rotate 12
//	tcMod rotate 8
	
    }
}

textures/sfx/helga_portal4
{
//	portal
	qer_editorimage textures/sfx/helga_portal4.tga
	surfaceparm trans
	surfaceparm nolightmap
	surfaceparm nonsolid
//	tesssize 128
		
	deformVertexes wave 160 sin 0 10 0 .3	// the chop
//	deformVertexes wave 25 sin 0 5 0 .1	// the chop
//	deformVertexes wave 64 sin .25 .25 0 .5

    {
        map textures/sfx/helga_portal4.tga
	blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
//	depthWrite
        tcMod rotate 12
//	tcMod rotate 8
	rgbgen identity
    }
}

textures/sfx/hportal_grad
{
//	portal
	qer_editorimage textures/sfx/hportal_grad.tga
	surfaceparm trans
	surfaceparm nolightmap
	surfaceparm nonsolid
//	tesssize 128
		
	deformVertexes wave 160 sin 0 10 0 .3	// the chop
//	deformVertexes wave 25 sin 0 5 0 .1	// the chop
//	deformVertexes wave 64 sin .25 .25 0 .5

    {
        map textures/sfx/hportal_grad.tga
	blendFunc GL_SRC_ALPHA GL_ONE_MINUS_SRC_ALPHA
//	depthWrite
        tcMod rotate 12
//	tcMod rotate 8
	
    }
}
